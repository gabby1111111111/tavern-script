        ...
        <div class="ganggang-voice-content">
          ...
          <template v-if="activeTab === 'sources'">
            <div class="ganggang-voice-actions">
              <button class="menu_button" type="button" @click="runtime.addProfile('openai-compatible')">
                新增来源
              </button>
            ...
            <datalist id="ganggang-voice-options">
              <option
                ...
                :key="`${voice.providerProfileId}:${voice.voiceId}`"
                :value="voice.voiceId"
              >
                {{ voice.name }}
              </option>
            ...
            <article class="ganggang-voice-card">
              ...
              <div v-for="effect in settings.soundEffects" :key="effect.id" class="ganggang-voice-grid">
                ...
                <label class="ganggang-voice-field">
                  <span>给 AI 的说明</span>
                  <textarea
                    v-model.trim="effect.description"
                    rows="2"
                    placeholder="例如：木门缓慢推开的吱呀声"
                  ></textarea>
                </label>
                <label class="ganggang-voice-field">
                  <span>试听音量</span>
              ...
              <li v-for="item in recentVoices" :key="item.id" class="ganggang-voice-card ganggang-voice-recent">
                ...
                <div class="ganggang-voice-actions">
                  ...
                  <button
                    ...
                    type="button"
                    :disabled="!item.blob"
                    @click="runtime.downloadRecent(item.id)"
                  >
                    下载
                  </button>
...
function roleLabel(role: CastRole): string {
  if (String(role).trim().toLocaleLowerCase() === 'user') return 'User';
